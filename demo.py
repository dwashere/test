...
try:
    dbstring = ('DRIVER={SQL Server};SERVER=FLAWLESSWASHERE\SQLEXPRESS;DATABASE=user3;Trusted_Connection=yes;')
except Exception:
...
query = f'SELECT *,price*(100-discount)/100 AS NewPrice FROM Product INNER JOIN Unit ON Unit=unit_id INNER JOIN Prod ON Prod=prod_id INNER JOIN Provider ON Provider=provider_id INNER JOIN Category ON Category=category_id'
...
data_tree = ttk.Treeview(tree_frame,columns=columns[1:],
                         style='data.Treeview')
data_tree.tag_configure('sale',background='lime')
